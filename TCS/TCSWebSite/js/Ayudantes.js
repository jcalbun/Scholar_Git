...
function search() {
    ...
    var strCodigo = document.getElementById("txtAyudanteId").value;
    ...
    Ayudante.getAyudantes(strCodigo, strRut, strNombre, strApellido, getAyudantes_CallBack);
}

function getAyudantes_CallBack(response) {
    // close Progress Bar
    ...
    xmlAyudantes = response.value;
    if (xmlAyudantes != '-1') {
        showAyudantes_SearchResults();
    }
}

function showAyudantes_SearchResults() {
    var xmlDoc = createDOM(xmlAyudantes);

    var Ayudantes = xmlDoc.getElementsByTagName("Ayudantes")[0].childNodes;
    ...
    for (var j = 0; j < Ayudantes.length; j++) {
        var row = fillHTMLRowWithAyudantesHeader(Ayudantes[j]);
        row.onclick = function () { onSearchResultRowClickAyudantes(this); }
...
function fillHTMLRowWithAyudantesHeader(xmlNode) {
    var tr = document.createElement('tr');
...
function onSearchResultRowClickAyudantes(row) {
...
function getCurrentNode(id) {
    var xmlDoc = createDOM(xmlAyudantes);
    var Ayudantes = xmlDoc.documentElement.getElementsByTagName('Ayudante');

    for (var j = 0; j < Ayudantes.length; j++) {
        if (id == Ayudantes[j].selectSingleNode('Id').childNodes[0].nodeValue) {
            return Ayudantes[j];
        }
    }
    return null;
