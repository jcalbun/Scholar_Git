...
function search() {
    ...
    ContactosConductor.getContactosConductor(txtConductor, txtContacto, txtDireccion, txtTelefono, getContactosConductor_CallBack);
}

function getContactosConductor_CallBack(response) {
    // close Progress Bar
...
function fillHTMLRowWithContactosHeader(xmlNode) {
    ...
    // Codigo
    var td = document.createElement('td');
    if (validExistTag(xmlNode.getElementsByTagName('IdConductor')[0])) {
        td.innerHTML = xmlNode.selectSingleNode('IdConductor').childNodes[0].nodeValue;
        td.width = "50px";
        td.align = "center";
    }
...
function resetCurrentPanel() {
    document.getElementById('txtCurrentIdConductor').value = '';
...
function getCurrentNode(id) {
    ...
    for (var j = 0; j < Contactos.length; j++) {
        if (id == Contactos[j].selectSingleNode('IdConductor').childNodes[0].nodeValue) {
            return Contactos[j];
        }
    }
    return null;
...
function displayCurrent(xmlNode) {
    if (xmlNode != null) {
        ...
        document.getElementById('txtCurrentIdConductor').value = xmlNode.selectSingleNode("IdConductor").childNodes[0].nodeValue;
...
function blockCurrent() {
    lockElements(new Array("txtCurrentIdConductor", "txtCurrentConductor", "txtCurrentDireccion1", "txtCurrentDireccion2", "txtCurrentTelefono1", "txtCurrentTelefono2", "txtCurrentEmail", "txtCurrentObs"));

    disableElement('txtCurrentIdConductor');
    disableElement('txtCurrentConductor');
